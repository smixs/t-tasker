...
@callback_router.message(EditTaskStates.editing_content)
...
@callback_router.message(EditTaskStates.editing_due_date)
async def handle_due_date_edit(
    message: Message,
    state: FSMContext
) -> None:
    ...
    try:
        # Update task in Todoist
        async with TodoistService(todoist_token) as todoist:
            await todoist.update_task(todoist_id, due_string=message.text)
        ...
        # Send success message
        await message.answer(
            f"✅ Дата задачи обновлена: <b>{message.text}</b>",
            parse_mode="HTML"
        )
