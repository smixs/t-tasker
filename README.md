...
**T-Tasker** - это полностью открытый Telegram-бот для создания задач в Todoist из голосовых и текстовых сообщений. Весь код доступен для проверки, все данные хранятся локально у вас, а конфиденциальные ключи никогда не покидают вашу инфраструктуру.

## 🔒 Безопасность превыше всего

### Почему T-Tasker безопасен?

1. **100% открытый код** - Весь исходный код доступен в этом репозитории. Вы можете изучить каждую строку и убедиться в отсутствии скрытых функций.

2. **Локальное хранение данных** - Все ваши данные (задачи, токены, настройки) хранятся в вашей собственной базе данных PostgreSQL. Никакие данные не отправляются на внешние серверы разработчиков.

3. **Шифрование токенов** - Todoist токены шифруются с использованием Fernet (симметричное шифрование) перед сохранением в базу данных.

4. **Изолированный запуск** - Docker контейнеры обеспечивают полную изоляцию приложения от вашей системы.

5. **Вы контролируете все ключи** - Все API ключи (Telegram, OpenAI, Deepgram, Todoist) принадлежат вам и хранятся только в вашем `.env` файле.

### Архитектура безопасности

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   Ваш Telegram  │────▶│   Ваш сервер    │────▶│  Внешние API    │
│                 │     │   (T-Tasker)    │     │                 │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                              │                        │
                              ▼                        ▼
